...
# 319 "C:/Program Files/Microchip/MPLABX/v5.50/packs/Microchip/PIC18Fxxxx_DFP/1.2.26/xc8\\pic\\include\\pic18_chip_select.h" 3
...
# 320 "C:/Program Files/Microchip/MPLABX/v5.50/packs/Microchip/PIC18Fxxxx_DFP/1.2.26/xc8\\pic\\include\\pic18_chip_select.h" 2 3
...
{0x78,0x64,0x68,0x78,0x64,0x66,0x66,0x7C},
{0x3C,0x62,0x60,0x60,0x60,0x62,0x62,0x3C},
{0x78,0x64,0x66,0x66,0x66,0x66,0x64,0x78},
{0x7E,0x60,0x60,0x7C,0x60,0x60,0x60,0x7E},
{0x7E,0x60,0x60,0x7C,0x60,0x60,0x60,0x60},
{0x3C,0x62,0x60,0x60,0x66,0x62,0x62,0x3C},
{0x66,0x66,0x66,0x7E,0x66,0x66,0x66,0x66},
{0x7E,0x18,0x18,0x18,0x18,0x18,0x18,0x7E},
{0x7E,0x18,0x18,0x18,0x18,0x18,0x1A,0x0C},
...
{0x7C,0x62,0x62,0x7C,0x60,0x60,0x60,0x60},
{0x38,0x64,0x64,0x64,0x64,0x6C,0x64,0x3A},
{0x7C,0x62,0x62,0x7C,0x70,0x68,0x64,0x62},
{0x1C,0x22,0x30,0x18,0x0C,0x46,0x46,0x3C},
{0x7E,0x18,0x18,0x18,0x18,0x18,0x18,0x18},
{0x66,0x66,0x66,0x66,0x66,0x66,0x66,0x3C},
{0x66,0x66,0x66,0x66,0x66,0x66,0x3C,0x18},
{0x81,0x81,0x81,0x81,0x81,0x99,0x99,0x66},
...
void DisplayLedMatrix()
{
    for(int i = 0;i<8;i++)
    {
        for(int j=0;j<8;j++)
        {
            PORTCbits.RC1 = (~characterHEX[1][i])>>j;
            clock_signal();
        }
        for(int j=0;j<8;j++)
        {
            PORTCbits.RC1 = row[i]>>j;
            clock_signal();
        }
        latch_signal();
        _delay((unsigned long)((5)*(4000000/4000000.0)));
    ...
    for(int i=0;i<38;i++)
    {
        if(kt == character[i])
        {
            for(int k = 0;k<8;k++)
            {
                for(int j=0;j<8;j++)
                {
                    PORTCbits.RC1 = (~characterHEX[i][k])>>j;
                    clock_signal();
            }
                for(int j=0;j<8;j++)
            {
                PORTCbits.RC1 = row[k]>>j;
                clock_signal();
            }
            latch_signal();
            _delay((unsigned long)((5)*(4000000/4000000.0)));
